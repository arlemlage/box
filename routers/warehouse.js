...
router.post("/view", auth, async (req, res) => {
    try {
        ...
        console.log(name);
        const data = new warehouse({ name, address, status })

        const warehouse_name = await warehouse.findOne({name:name});
        if(warehouse_name){
...
router.post("/view/:id", auth, async (req, res) => {
    try {
        ...
        const purchases_data = await purchases.find({ warehouse_name : data.name})
